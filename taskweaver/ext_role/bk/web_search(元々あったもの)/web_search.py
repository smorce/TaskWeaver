# このロールでやっていることを一言で表すと
# 221 行目の post_proxy.update_message メソッドを叩くだけ
...
class WebSearch(Role):
    ...
    # 処理の流れ
    # セッションが開始されたら最後の ROUND のデータから、最後のPOSTを取得してくる
    # ラストPOSTからユーザークエリを抽出して、search_query で URL のリストを取得して
    # POST のメッセージを browse_page でアップデートして
        # 具体的には、URL のリストを受け取って、URL から Webページのテキストを抽出して、テキストをチャンキングして、ユーザークエリに近いチャンクを3つ返す
    # POST を終了する
    # なので、やっていることは、ユーザークエリからスクレイピングAPIを叩いて、抽出されたテキストから、セマンティックサーチして返すだけ。
    # そういう意味では、テキストメッセージで返信する機能だけ(post_proxy.update_message をするだけ)を提供している。
